...
mainKeyboard = ReplyKeyboardMarkup(
    [['نهم', 'دهم'], ['یازدهم', 'دوازدهم'], ['ℹ️ معرفی']],
    resize_keyboard=True)
choiceKeyboard = ReplyKeyboardMarkup([['📚 کتابچه', '📝 آزمونچه'], ['🔙 بازگشت']],
                                     resize_keyboard=True)
readyKeyboard = ReplyKeyboardMarkup([['✅ معلومه که آماده‌م!'], ['🔙 بازگشت']],
                                    resize_keyboard=True)
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if text == '/start':
        ...
        if text == "دوازدهم":
            message.reply_text(
                "❌ فعلاً محتوای خاصی برای پایه دوازدهم آماده نیست.")
        else:
            message.reply_text(
        ...
        if not grade:
            message.reply_text("⛔ لطفاً ابتدا پایه تحصیلی خود را مشخص کن.")
            return
        ...
        message.reply_text(bookMessage if mode == 'book' else examMessage)
        context.bot.send_message(chat_id=chat_id, text=sampleReminder)
        ...
        if os.path.exists(filepath):
            try:
                ...
                message.reply_text(
                    "❌ مشکل در ارسال فایل. لطفاً به ادمین پیام بده.")
        else:
            message.reply_text(
                "❌ فایل نمونه موجود نیست. لطفاً به ادمین پیام بده.")

    elif text == "✅ معلومه که آماده‌م!":
        grade, mode = user_grades.get(chat_id), user_mode.get(chat_id)
        if not get_full_filename(grade, mode):
            message.reply_text("⛔ نسخه کامل برای این پایه فعلاً آماده نیست.",
                               reply_markup=backOnlyKeyboard)
        else:
            user_ready[chat_id] = True
            message.reply_text(paymentInfo)
        ...
        message.reply_text("بازگشتی موفقیت‌آمیز 😄", reply_markup=mainKeyboard)

    elif user_ready.get(chat_id):
        try:
            context.bot.send_message(ADMIN_ID,
                                     f"📥 رسید از کاربر {chat_id} دریافت شد:")
            ...
            context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("تأیید پرداخت",
                                         callback_data=f"confirm_{chat_id}")
                ]]))
            message.reply_text("✅ رسیدت توسط بات برای ادمین ارسال شد.")
            user_ready[chat_id] = False
        except Exception as e:
            print(f"[ERROR] رسید ارسال نشد: {e}")
            message.reply_text("❌ خطا در ارسال رسید. لطفاً دوباره امتحان کن.")

    elif chat_id != str(ADMIN_ID):
        message.reply_text("📩 پیام شما ثبت شد و به ادمین ارسال می‌شود.")
        context.bot.forward_message(chat_id=ADMIN_ID,
                                    from_chat_id=int(chat_id),
                                    message_id=message.message_id)
...
def handle_callback_query(update: Update, context: CallbackContext):
    ...
    if query.data.startswith("confirm_") and str(
            query.message.chat.id) == str(ADMIN_ID):
...
def main():
    ...
    dp.add_error_handler(
        lambda update, context: print(f"[ERROR LOG] {context.error}"))
